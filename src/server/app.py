...
# Dictionary to store active tickets (phone_number -> ticket_id)
active_tickets = {}
...
try:
    ...
    # Reference to the processed calls in Firebase
    processed_calls_ref = db.reference('processed_calls')
    # Reference to active tickets in Firebase
...
def create_zendesk_ticket():
    ...
    try:
        ...
        # Check if event-call pair exists in Firebase
        ...
        # Store the event-call pair in Firebase
        ...
        if event == 'call_started':
            # Create initial ticket when call starts
...
"""
            ...
            if result and 'ticket' in result:
                # Store the ticket ID in Firebase
            ...
            # Update existing ticket when call ends
            ...
            if not ticket_id:
                ...
                # If no active ticket found, create a new one
                start_time = datetime.utcfromtimestamp(data['call']['start_timestamp'] / 1000).strftime('%Y-%m-%d %H:%M:%S')
...
"""
                ...
                # Update existing ticket with call details
                end_time = datetime.utcfromtimestamp(data['call']['end_timestamp'] / 1000).strftime('%Y-%m-%d %H:%M:%S')
...
"""
                ...
                # Remove the ticket from active tickets in Firebase
                if active_tickets_ref:
